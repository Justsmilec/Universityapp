...
public ws: any;
  ...
  // metoda connect qe ben lidhjen me serverin e websocket 
    ...
    async getList(): Promise<Subjectpost[]>{
        await this.delay(2000);

        return this.subjectsposts;
    }
    // per save te appliances te re
    onSendSave(destination: String, post: Subjectpost) {  
        this.getAllSubjectPosts();;
        this.resolveAfter2Seconds(20).then(() => {
    ...
    onMessageReceived(message:any) {
       this.subjectsposts = (JSON.parse(message.body));   // element eshte brenda obj myappliance qe mban listen appliance(dashboard)
